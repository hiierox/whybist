...
@pytest.mark.asyncio
async def test_refresh_success_200(client, user_factory):
    user, _ = user_factory

    login_response = await client.post(
        '/login', json={'identifier': user['email'], 'password': user['password']}
    )
...
@pytest.mark.asyncio
async def test_refresh_unauthorized_401(client, user_factory):
    user, _ = user_factory

    login_response = await client.post(
        '/login', json={'identifier': user['email'], 'password': user['password']}
    )
    wrong_token = login_response.json()['access_token']

    response = await client.post('/refresh', json={'refresh_token': wrong_token})
